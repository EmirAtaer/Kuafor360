...
const FALLBACK_SERVICES = [
  { id: 1001, name: 'Saç Kesimi', price: 0 },
  { id: 1002, name: 'Sakal Kesimi', price: 0 },
  { id: 1003, name: 'Saç + Sakal', price: 0 },
];
...
function renderServiceOptions() {
  ...
  select.innerHTML = '';
  const list = state.services.length ? state.services : FALLBACK_SERVICES;
  const placeholder = state.services.length ? 'Hizmet seçiniz' : 'Hazır hizmetlerden birini seçin';
...
async function handleBookingSubmit(event) {
  ...
  if (response) {
    ...
    if (state.adminLoggedIn) {
      dom.admin.feedback.textContent = `Yeni randevu: ${state.customer.full_name} (${state.customer.phone}) ${state.customerDate} ${
        bookedSlot.start
      }`;
    }
  } else {
